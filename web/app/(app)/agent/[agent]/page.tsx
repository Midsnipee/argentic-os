...
export default function AgentChatPage() {
  ...
  const addTask = () => {
    ...
    setTaskInput("");
    setMessages((prev) => [...prev, { role: "user", text: `📋 Nouvelle tâche : ${task.title}` }]);
    // Envoie la tâche à l'agent
    setLoading(true);
    api.chat(agentId as string, task.title).then(async (res) => {
      const data = await res.json();
      const reply = data.success ? data.response : data.error || "Erreur";
      setMessages((prev) => [...prev, { role: "agent", text: reply }]);
      setTasks((prev) => prev.map((t) => t.id === task.id ? { ...t, status: "done" as const } : t));
      setLoading(false);
    }).catch(() => {
      setLoading(false);
    });
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border-light)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8, fontSize: "0.7rem", color: "var(--text-dim)" }}>
              {pendingTasks > 0 && <span style={{ color: "#f59e0b" }}>{pendingTasks} en attente</span>}
          ...
          <form onSubmit={(e) => { e.preventDefault(); addTask(); }}
            ...
            <button type="submit" disabled={!taskInput.trim()}
              ...
              style={{ padding: "8px 14px", fontSize: "0.8rem", opacity: !taskInput.trim() ? 0.4 : 1 }}>
              +
            </button>
          </form>
              ...
              {tasks.map((task) => (
                <div key={task.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "0.65rem", color: "var(--text-muted)" }}>{task.created}</div>
                  </div>
                  <button onClick={() => deleteTask(task.id)}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", fontSize: "0.7rem", padding: 2 }}>
                    ✕
                  </button>
                </div>
              ))}
        ...
        <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border-light)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: "0.75rem" }}>
            <div>
              <div style={{ color: "var(--text-muted)" }}>Tours max</div>
              <div style={{ fontWeight: 600 }}>{agent.max_turns}</div>
            </div>
            <div>
              ...
              <select
                ...
                style={{
                  ...
                  background: "#fff", color: "var(--text)", fontWeight: 500, marginTop: 2,
                }}
              >
            ...
            <div>
              <div style={{ color: "var(--text-muted)" }}>Contexte</div>
              <div style={{ fontWeight: 600 }}>{currentModel?.context?.toLocaleString() || "—"} tk</div>
            </div>
            <div>
              <div style={{ color: "var(--text-muted)" }}>Coût</div>
              <div style={{ fontWeight: 600 }}>{currentModel?.cost || "—"}</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "16px 20px", flex: 1 }}>
          ...
          {agent.workdir ? (
            <code style={{
              fontSize: "0.75rem", background: "var(--bg)", padding: "6px 10px",
              borderRadius: 6, color: "var(--primary)", display: "block",
            }}>
              {agent.workdir}
            </code>
          ) : (
            <div style={{ fontSize: "0.78rem", color: "var(--text-muted)", fontStyle: "italic" }}>
              Dossier par défaut : /opt/data/argentic-os/projects/
            </div>
          )}
        </div>
      </div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%" }}>
              <div style={{ textAlign: "center", color: "var(--text-dim)" }}>
                ...
                <div style={{ fontSize: "0.95rem" }}>
                  Créez une tâche ou envoyez un message
                </div>
              </div>
            </div>
          ) : (
